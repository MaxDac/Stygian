defmodule StygianWeb.GuideLive.GuideRulesLive do
  ...
  def rules(assigns) do
    ~H"""
    <.guide_p>
      <b>Stygian</b> è un gioco di ruolo a turni, strettamente via chat. Ogni giocatore avrà la possibilità di creare e gestire un solo personaggio alla volta, e potrà decidere di abbandonarlo, perdendo tutti i punti esperienza accumulati, oppure crearne uno nuovo quando il precedente personaggio morirà o scomparirà in azione - in questo conservando la totalità dei suoi punti esperienza.
    </.guide_p>

    <.guide_p>
      Un utente potrà iscriversi ed osservare il gioco in chat prima di creare il suo personaggio, ma la creazione del personaggio è subordinata alla conferma dell'utente. Una volta confermata la mail data in fase di creazione, si potrà procedere alla creazione del personaggio, il cui regolamento è espresso nella sezione relativa alla <.link patch={~p"/guide/creation"} class="font-bold underline underline-offset-8">Creazione</.link>.
    </.guide_p>

    <.h1>Gioco in chat</.h1>

    <.guide_p>
      Il gioco avverrà principalmente in chat, e sarà diviso in turni. Ogni personaggio che vorrà prendere parte ad una giocata dovrà aspettare il proprio turno per scrivere la sua azione. L'interpretazione dell'azione <b>non dovrà lasciare spazio ai pensieri del personaggio</b> si dovrà cioè limitare solamente a ciò che del personaggio è visibile: ad esempio, non si potrà dire in un'azione: <i>Il personaggio pensa a quanto il suo interlocutore sia del tutto orripilante</i>, ma si potrà dire <i>Il personaggio non riesce a celare una espressione di profondo sdegno e orrore mentre discute col suo interlocutore</i>. Il giocatore, poi, avrà a disposizione per ogni azione del personaggio un numero massimo di <span class="font-bold underline">1200 caratteri</span>, ed un minimo di 200 caratteri. 
    </.guide_p>
